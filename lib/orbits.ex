defmodule Orbits do
  ...
  @doc """
  iex> Orbits.from("COM)A  A)B  B)C") |> Orbits.reverse_map
  %{"A" => "COM", "B" => "A", "C" => "B"}
  """
  @spec reverse_map(universe) :: map
  def reverse_map(universe) do
    orbits =
      universe
      |> Enum.filter(fn
        %Orbit{} -> true
    ...
    orbits
    |> Enum.reduce(%{}, fn orbit, map ->
      map |> Map.put(orbit.orbiting.name, orbit.orbited.name)
    end)
  end
  ...
  def find_path_to_root(universe, orbiting_name) do
    reverse_map = universe |> reverse_map
    do_find_path_to_root(reverse_map, orbiting_name)
  end

  defp do_find_path_to_root(reverse_map, orbiting_name, path \\ []) do
    orbited_name = reverse_map |> Map.get(orbiting_name)

    if orbited_name do
      do_find_path_to_root(reverse_map, orbited_name, path ++ [orbited_name])
    else
      path
    end
  end

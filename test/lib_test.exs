defmodule LibTest do
  ...
  it, individually calculate the fuel needed for the mass of each module (your
  ...
  test :day_03_part_2 do
    shortest_path =
      ...
      |> String.split("\n")
      |> Enum.map(&Grid.route/1)
      |> Grid.intersection_shortest_path()

    assert shortest_path == 14_746
  end
  ...
  test :day_04 do
    candidates =
      273_025..767_253
      |> Password.find_candidates()
      |> length

    assert candidates == 910
  end
  ...
  test :day_04_part_2 do
    candidates =
      273_025..767_253
      |> Password.find_candidates_with_one_dup()
      |> length

    assert candidates == 598
  end
  ...
  test :day_05_part_2 do
    ...
    output =
      %Program{code: code}
      |> Program.with_input(5)
      |> Program.run()
      |> Program.output()

    assert output === 15_163_975
  end
  ...
  test :day_07 do
    ...
    amp_set =
      %Program{code: code}
      |> AmplifierSet.new(amplifier_count: 5)

    {permutation, output} =
      amp_set
      |> AmplifierSet.find_best_phase_settings()

    assert permutation == [2, 1, 4, 3, 0]
    assert output == 118_936
  end
  ...
  test :day_07_part_2 do
    ...
    assert permutation == [9, 7, 6, 5, 8]
    assert output == 57_660_948
  end
end
